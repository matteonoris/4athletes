...
class DailyReadinessDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<Map<String, dynamic>> metricsToShow = isSleep 
      ...
      : [
          {'key': 'rhr', 'label': 'Battiti a Riposo (bpm)', 'icon': Icons.favorite},
          {'key': 'hrv', 'label': 'HRV (ms)', 'icon': Icons.monitor_heart},
          {'key': 'temp', 'label': 'Temperatura (°C)', 'icon': Icons.thermostat},
          {'key': 'resp', 'label': 'Frequenza Respiratoria (rpm)', 'icon': Icons.air},
          {'key': 'spo2', 'label': 'Ossigenazione (%)', 'icon': Icons.bloodtype},
        ];

    return Scaffold(
      appBar: AppBar(
        title: Text('Dettagli $title', style: const TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: ListView(
        ...
        children: [
          ...
          Text('Metriche di oggi', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          ...
          ...metricsToShow.map((metric) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                title: Text(metric['label'], style: const TextStyle(color: AppTheme.textHighEmphasis, fontWeight: FontWeight.w600)),
                trailing: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Text(mVal.toStringAsFixed(1), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
